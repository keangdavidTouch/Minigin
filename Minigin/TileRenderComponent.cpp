...
kd::TileRenderComponent::TileRenderComponent(const SpriteInfo& info, int cols, int rows) 
	: m_Rows(rows) 
	, m_Cols(cols) 
	, m_SpriteSourceRect()
	, m_DescRects(std::vector<SDL_Rect>())
{
	SpriteSheetManager::GetInstance().GetSourceRectByRef(info, m_SpriteSourceRect);
	m_Texture = SpriteSheetManager::GetInstance().GetTexture(info.SpriteSheetID);
}
...
void kd::TileRenderComponent::Initialize()
{
	...
	for (int  i = 0; i < m_Rows; i++)
	{
		...
		{
			m_DescRects.push_back(SDL_Rect(desc));
			desc.x += desc.w;
		}
